  ...
  try {
    ...
    if (password === employee.password) {
      ...
      res
        ...
        .cookie(
          "employee_details",
          { id: employee._id, auth_token: `bearer ${token}` },
...
module.exports.getEmployeeById = async (req, res) => {
  try {
    ...
    if (employee) {
      const { _id, name, email, mobile, role, skill, status, profilePhoto } =
        employee;
    ...
    return res
      .status(500)
      .json({ message: "Server side error", details: error.message });
  }
};
...
exports.updateEmployeeById = async (req, res) => {
  try {
    ...
    if (newEmployeeDetails.password) {
      newEmployeeDetails.password = bcrypt.hashSync(
        newEmployeeDetails.password,
        10
      );
    }
    const employee = await employeeModel.findByIdAndUpdate(
  ...
  try {
    const newEmployee = {
      profilePhoto: {
        imageUrl: path.join(
          __dirname,
          `../uploads/employees/${req.params.employeeId}/${req.filename}`
        ),
      },
    };
...
exports.deleteProfileImage = async (req, res) => {
  try {
    await removeDirectory(
      path.join(__dirname, `../uploads/employees/${req.params.employeeId}`)
    );
    const result = await employeeModel.findByIdAndUpdate(
      req.params.employeeId,
...
const createDirIfNotExists = async (filepath, req) => {
  ...
  await fs.mkdir(`${filepath}/${formatDate()}/${req.body.data.employeeId}`, {
    recursive: true,
  });
  await fs.mkdir(
    `${filepath}/${formatDate()}/${req.body.data.employeeId}/${
      req.params.dailyRecordId
    }`,
    { recursive: true }
  );
...
const moveFiles = async (filepath, req) => {
  ...
  const oldFiles = await fs.readdir(
    `${filepath}/${formatDate()}/${req.body.data.employeeId}/${
      req.params.dailyRecordId
    }`
  );
  ...
  for (const file of files) {
    ...
    const newPath = path.join(
      ...
      `${oldFiles.length + 1}${path.extname(file)}`
    );
...
exports.getAllDailyRecordsByEmployeeId = async (req, res) => {
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        message: `Records with siteId ${req.params.siteId} not found`,
      });
    }
